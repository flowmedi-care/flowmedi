    ...
    >
      {letters.map((letter) => (
        <button
          ...
          className={cn(
            "h-4 w-4 rounded flex items-center justify-center hover:text-primary hover:bg-primary/10",
            activeLetter === letter && "text-primary bg-primary/10"
          )}
        >
          {letter}
        </button>
...
export function ContactList({
  ...
  return (
    <div className={cn("flex gap-2 min-h-0", className)}>
      <div className="flex-1 min-w-0 overflow-y-auto max-h-[calc(100vh-320px)] space-y-4 pr-1">
        {children}
      </div>
      {sideRail && (
        <div className="hidden sm:flex flex-col shrink-0 border-l border-border/40 pl-1">
          {sideRail}
        </div>
      )}
    </div>
