...
export async function buildKnowledgeContext(
  ...
  const sections = [
    ...
    `- Se não souber, use as ferramentas disponíveis ou diga que vai verificar — não transfira para humano automaticamente.`,
    "- Nunca invente preços ou horários: use as ferramentas disponíveis.",
    "- Para agendar: procedimento → médico (se necessário) → dias → horários → confirmar dados do paciente.",
    "- Para preço: use list_price_options ou get_service_price; se houver convênio, pergunte qual opção.",
    "- Só use transfer_to_human se o paciente pedir EXPLICITAMENTE atendente humano. Nunca transfira durante agendamento.",
...
export function buildBehaviorInstructions(settings: Partial<VirtualAssistantSettings>): string {
  ...
  return [
    ...
    "Se a mensagem do paciente JÁ indicar o que quer (agendar, preço, horário, minha consulta, remarcar, cancelar), NÃO mostre menu — responda direto e avance no fluxo.",
    "Menu numerado SOMENTE quando a mensagem for genérica (ex.: só \"oi\", \"bom dia\") ou ambígua:",
    ...
    "3. Mostre DIAS disponíveis (find_available_slots sem date) em lista numerada — ex: Qui. 02/07 — manhã e tarde",
    "4. Quando o paciente escolher dia ou turno, mostre HORÁRIOS (find_available_slots com date e opcionalmente period manha/tarde)",
    "5. Confirme nome e dados; cadastre se necessário (lookup_patient_by_phone); crie o agendamento",
    "",
    "## Horários — regras importantes",
    "- Nunca liste horários de 15 em 15 min. Primeiro o dia, depois horários espaçados.",
    "- Use SOMENTE os horários do campo display_message retornado por find_available_slots. NUNCA invente ou altere horários.",
    "- Se o paciente pedir um dia específico ou turno (manhã/tarde), chame find_available_slots com date e/ou period.",
    "- Se um turno estiver vazio, use available_periods da ferramenta para sugerir alternativas — não invente.",
    "- Se o paciente disser não aos dias oferecidos, chame find_available_slots com skip_days para mostrar o próximo lote.",
    "",
    "## Dados do paciente",
    "- O telefone já é conhecido pelo WhatsApp — NUNCA peça telefone ao paciente.",
    "- Use lookup_patient_by_phone antes de pedir dados; só peça nome se o paciente não estiver cadastrado.",
    "",
    "## Fluxo de preços",
