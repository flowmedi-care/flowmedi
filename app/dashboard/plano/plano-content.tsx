...
export function PlanoContent(props: PlanoContentProps) {
  ...
  const statusText = isPro && !isCancelScheduled
    ? "Assinatura ativa"
    : isPro && isCancelScheduled
      ? "Cancelamento agendado para o fim do período"
      : isProPastDue
        ? "Pagamento atrasado - atualize o cartão para manter o acesso"
        : isCanceled
          ? "Assinatura cancelada"
          : "Plano sem assinatura ativa";
        ...
        <CardContent className="space-y-5">
          {subscriptionInfo?.cancelAtPeriodEnd && subscriptionInfo?.currentPeriodEnd && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-900/50 dark:bg-amber-950/30 px-4 py-3 text-sm text-amber-800 dark:text-amber-200">
              Seu acesso continua até {formatDate(subscriptionInfo.currentPeriodEnd)} ({formatDaysLeft(subscriptionInfo.currentPeriodEnd)} dias).
            </div>
          )}
            ...
            <form onSubmit={onPaymentSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="tax_id_value">{taxIdType === "cpf" ? "CPF" : "CNPJ"}</Label>
                    <Input
                      id="tax_id_value"
          ...
          {!isPro && !showCheckoutForm && upgradePlans.length > 0 && (
            <div className="space-y-3">
              ...
              {effectiveCheckoutPlan?.stripe_price_id ? (
                ...
                <div className="rounded-lg border border-border bg-muted/30 p-4 text-sm text-muted-foreground">
                  Este plano requer contato comercial.
                </div>
              )}
            </div>
          )}
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  {otherPlans.map((p) => (
                    <button
                      ...
                      className="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-md text-xs font-medium h-9 px-3 border border-input bg-background shadow-sm hover:bg-accent hover:text-accent-foreground transition-colors disabled:pointer-events-none disabled:opacity-50"
                    >
                      {loadingCheckout ? (
                        <>
                      ...
                      <tr key={inv.id} className="border-b">
                        ...
                        <td className="py-2">
                          {inv.hosted_invoice_url ? (
                            <a href={inv.hosted_invoice_url} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline inline-flex items-center gap-1">
                              Ver fatura
                      ...
                      <tbody>
                        {messageStats.recentMessages.map((msg) => (
                          <tr key={msg.id} className="border-b">
                            <td className="py-2 pr-4">{new Date(msg.sent_at).toLocaleString("pt-BR")}</td>
