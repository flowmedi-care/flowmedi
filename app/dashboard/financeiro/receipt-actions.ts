...
import { renderReceiptPdfBuffer, type ReceiptPdfLine } from "@/lib/financeiro/receipt-pdf";
...
async function buildReceiptPdfPayload(
  ...
  let comandaTotal: number | null = null;
  let comandaRemainder: number | null = null;
  if (comandaId) {
    const { data: cmd } = await supabase
      ...
      .maybeSingle();
    if (cmd) {
      comandaTotal = Number(cmd.total_amount);
      comandaRemainder = Math.max(0, comandaTotal - Number(cmd.paid_amount));
    }
  }
  ...
  return {
    ...
    comanda_remainder: comandaRemainder,
...
export async function getReceiptPrintData(receiptId: string) {
  ...
  let comandaTotal: number | null = null;
  let comandaPaid: number | null = null;
  if (comandaId) {
    const { data: cmd } = await supabase
      .from("comandas")
      .select("total_amount, paid_amount")
    ...
    if (cmd) {
      comandaTotal = Number(cmd.total_amount);
      comandaPaid = Number(cmd.paid_amount);
    }
  }
  ...
  return {
    ...
    data: {
      ...
      comanda_total: comandaTotal,
      comanda_remainder:
        comandaTotal != null && comandaPaid != null
          ? Math.max(0, comandaTotal - comandaPaid)
          : null,
    },
  };
}
