...
async function fetchQuoteById(
  ...
  const [{ data: items, error: itemsError }, patientRes, pipelineRes, professionalRes] =
    await Promise.all([
      supabase
        .from("quote_items")
        .select("*")
      ...
      data.professional_id
        ? supabase.from("profiles").select("full_name").eq("id", data.professional_id).maybeSingle()
        : Promise.resolve({ data: null, error: null }),
    ]);
  ...
  const detail = mapQuoteDetail(enriched, items ?? []);
  if (professionalRes.data?.full_name) {
    detail.professional_name = professionalRes.data.full_name;
  }
...
export async function listQuotes(): Promise<{
  ...
  const { data, error } = await ctx.supabase
    ...
    .select(
      ...
      recipient_name, patient_id, pipeline_id,
  ...
  const pipelineIds = [
    ...new Set(
      (data ?? [])
        .map((row) => row.pipeline_id as string | null)
  ...
  if (pipelineIds.length > 0) {
    ...
    for (const p of pipelines ?? []) {
      pipelineNames.set(p.id, p.name ?? p.email ?? "Lead");
    }
  }
  ...
  const rows: QuoteListItem[] = (data ?? []).map((row) => {
    ...
    return {
      ...
      recipient_display:
        patient?.full_name ??
        (row.pipeline_id ? pipelineNames.get(row.pipeline_id as string) : null) ??
        row.recipient_name ??
        "—",
