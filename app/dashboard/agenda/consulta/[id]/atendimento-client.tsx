...
import { ComandaBillingPanel } from "../../components/comanda-billing-panel";
import { AppointmentOperationalStepper } from "../../components/appointment-operational-stepper";
...
export function AtendimentoClient({
  ...
  const [operationalState, setOperationalState] = useState<AppointmentOperationalState | null>(
    null
  );
  ...
  const load = useCallback(async () => {
    ...
    const [res, opRes] = await Promise.all([
      getAppointmentConsumption(appointmentId),
      getAppointmentOperationalState(appointmentId),
    ]);
    if (res.error) {
    ...
    if (!opRes.error && opRes.data) setOperationalState(opRes.data);
  ...
  function handleStepAction(step: number) {
    if (step === 4 && !isComandaFinalized) {
      document.getElementById("comanda-billing-panel")?.scrollIntoView({ behavior: "smooth" });
    }
  }
  ...
  const showStepper = mode === "full" && operationalState;
  ...
  return (
    <div className="space-y-4">
      {showStepper && operationalState && (
        <AppointmentOperationalStepper state={operationalState} onStepAction={handleStepAction} />
      )}

      {!encounterStatus && canEdit && !autoStart && (
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm">
          ...
          {comanda && (
            <Badge variant="secondary">
              ...
              {comanda.remainder > 0 && ` · falta ${comanda.remainder.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}`}
            </Badge>
          )}
        </div>
      )}
