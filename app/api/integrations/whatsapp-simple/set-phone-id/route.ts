...
/**
 * Atualiza o Phone Number ID manualmente para WhatsApp Simples
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (fetchError || !integration) {
      return NextResponse.json(
        { error: "Integração WhatsApp Simples não encontrada. Conecte primeiro." },
        { status: 404 }
      );
    }
    ...
    console.error("Erro ao definir Phone Number ID (WhatsApp Simples):", error);
    return NextResponse.json(
