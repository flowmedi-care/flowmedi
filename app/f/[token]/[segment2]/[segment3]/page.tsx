...
export default async function FormularioComSlugPage({
  ...
  params: Promise<{ token: string; segment2: string; segment3: string }>;
}) {
  try {
    const { token: clinicSlug, segment2: formSlug, segment3: patientSlug } = await params;

    if (
      !clinicSlug ||
      !formSlug ||
      !patientSlug ||
      typeof clinicSlug !== "string" ||
      typeof formSlug !== "string" ||
      typeof patientSlug !== "string"
    ) {
      notFound();
    }
