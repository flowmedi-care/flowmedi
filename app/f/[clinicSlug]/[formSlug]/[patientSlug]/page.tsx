...
export default async function FormularioComSlugPage({
  ...
  params: Promise<{ clinicSlug: string; formSlug: string; patientSlug: string }>;
}) {
  try {
    const { clinicSlug, formSlug, patientSlug } = await params;
    
    if (!clinicSlug || !formSlug || !patientSlug || 
        typeof clinicSlug !== 'string' || typeof formSlug !== 'string' || typeof patientSlug !== 'string') {
      notFound();
    }

    const supabase = await createClient();
    
    // Buscar instância pelo slug composto (clinicSlug/formSlug/patientSlug)
