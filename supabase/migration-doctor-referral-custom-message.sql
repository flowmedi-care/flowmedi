...
-- Migra tabela antiga (com code) para novo schema (custom_message).
-- Se a tabela já foi criada com custom_message (migration-doctor-referral-codes atualizada), esta migration é no-op.
--
-- Execute após: migration-doctor-referral-codes.sql

-- Só altera se a coluna code ainda existir (schema antigo)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'doctor_referral_codes' AND column_name = 'code'
  ) THEN
    ...
    -- Adicionar coluna custom_message
    ALTER TABLE public.doctor_referral_codes
  ...
  -- Garantir constraint UNIQUE(clinic_id, doctor_id)
  ALTER TABLE public.doctor_referral_codes
